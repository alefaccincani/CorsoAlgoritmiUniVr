...
\begin{esempio}[{MergeSort su $A=[5,2,4,6,1,3]$}]
	\begin{center}
		\begin{tikzpicture}[
			level distance=13mm,
		...
		\end{tikzpicture}
	\end{center}
	Le foglie sono i sotto-array di dimensione 1 (già ordinati).
	La fase di \emph{merge} risale l'albero dal basso verso la radice,
	fondendo i fratelli a ogni livello. Con $n=6$ ci sono $\lceil\lg 6\rceil
	= 3$ livelli di merge, ognuno di costo $\Theta(n)$.
\end{esempio}
