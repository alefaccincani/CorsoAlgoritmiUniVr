...
\begin{esempio}[Rappresentazione Heap: array e albero]
	Dato l'array $A = [16, 14, 10, 8, 7, 9, 3, 2, 4, 1]$, la corrispondente
	struttura ad albero è:
	...
	\noindent Le posizioni dell'array corrispondono ai nodi per livelli
	(BFS):
	
	\begin{center}
		\begin{tikzpicture}
			\foreach \val/\idx in {16/1,14/2,10/3,8/4,7/5,9/6,3/7,2/8,4/9,1/10}{
				\draw (\idx-1,0) rectangle (\idx,0.7);
				\node at (\idx-0.5, 0.35) {\val};
				\node[font=\tiny] at (\idx-0.5, -0.25) {\idx};
			}
		\end{tikzpicture}
	\end{center}
	
	\noindent \textbf{Regola di navigazione} (indice 1-based):
	\[
	\mathrm{left}(i) = 2i, \quad
	\mathrm{right}(i) = 2i+1, \quad
	\mathrm{parent}(i) = \lfloor i/2 \rfloor.
	\]
	Il nodo $i=2$ (valore 14) ha figlio sinistro in posizione $4$ (valore 8)
	e figlio destro in posizione $5$ (valore 7).
\end{esempio}
